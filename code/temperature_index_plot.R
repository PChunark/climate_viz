...
# 2. Draw two lines with different color. We created dummies variable in color by using 'aes(color = "1")'  
read_csv("data/GLB.Ts+dSST.csv", skip = 1, na = "***") %>%
  select(year = Year, t_diff = `J-D`) %>% # Change colums name
  ggplot(aes(x = year, y = t_diff)) + 
  ...
  theme(
    ...
    legend.key.height = unit(10, "pt"), # Adjust the line spacing between two lines in the legend area
    legend.margin = margin(0,0,0) #Remove the margin around legend box
    )

# 3. Change lines eleement in a legend box to square by turning off the show.legend in the geom_line and geom_smooth
read_csv("data/GLB.Ts+dSST.csv", skip = 1, na = "***") %>%
  select(year = Year, t_diff = `J-D`) %>% # Change colums name
  ggplot(aes(x = year, y = t_diff)) + 
  geom_line(aes(color = "1"), size = 0.5, show.legend = FALSE)+ # Add line color and size, since color is static the dummy variable is created 'aes(color = "1")'
  geom_point(fill = "white", aes(color = "1"), shape =21, show.legend = TRUE)+ # Add point fill color, border color, shape of points, since color is static the dummy variable is created 'aes(color = "1")', remove point from legend
  geom_smooth(se = FALSE, aes(color = "2"), size = 0.5, span = 0.15, show.legend = FALSE)+ # smooth the data, remove standard errors, change line color and size, give smoothness using 'span', since color is static the dummy variable is created 'aes(color = "2")' 
  scale_x_continuous(breaks = seq(1880, 2020,20), expand = c(0,0))+ #Adjust the x axis, remove space between x and y axis
  scale_y_continuous(limits = c(-0.5, 1.5), expand = c(0,0))+ #Adjust the y axis
  scale_color_manual(name = NULL,             # When we added the dummy 'aes(color = "1")', we need to specify the color to dummies.
                     breaks = c(1,2),         # There are 2 dummies 1,2
                     values = c("grey", "black"),   # assign color value
                     labels = c("Annual mean", "Lowess smoothing"), #Assign label
                     guide  = guide_legend(override.aes = list(shape = 15, size = 5)) #Override a shape in a legend area by square (shape = 15)
                     )+ 
